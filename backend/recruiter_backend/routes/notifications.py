...
async def register_device(payload: DeviceTokenPayload, current_recruiter: Recruiter = Depends(get_current_recruiter), db: Session = Depends(get_db)):
    token = db.query(DeviceToken).filter(DeviceToken.device_token == payload.device_token).first()
    if token:
        token.user_type = "recruiter"
        token.user_id = current_recruiter.id
...
async def unregister_device(payload: DeviceTokenPayload, current_recruiter: Recruiter = Depends(get_current_recruiter), db: Session = Depends(get_db)):
    token = db.query(DeviceToken).filter(DeviceToken.device_token == payload.device_token, DeviceToken.user_id == current_recruiter.id).first()
    if not token:
...
@router.post("/send")
async def send_notification(payload: NotificationPayload, current_recruiter: Recruiter = Depends(get_current_recruiter), db: Session = Depends(get_db)):
    tokens = db.query(DeviceToken).filter(DeviceToken.user_type == "recruiter", DeviceToken.user_id == current_recruiter.id, DeviceToken.active == True).all()
    if not tokens:
